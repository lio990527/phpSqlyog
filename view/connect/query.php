...
<head>
    ...
    <script type="text/javascript">
    	...
    	function excute(){
    		var sqls = window.getSelection().toString();
    		if(sqls == ''){
    			sqls = $('textarea#input').val();
    		}
    		sqls = sqls.split(';');
    		for(var i in sqls){
    			sqls[i] = sqls[i].replace(/(^\s+)/g,'').replace(/\n/g, '');
    		}
    		sqls = sqls.join('__', sqls);
    		console.log(sqls);
    		document.forms['query'].sql.value = sqls;
    		document.forms['query'].submit();
    	}
    	
    	function checkKeyword(dom){
    		var start = dom.selectionStart;
    		var stop = dom.selectionEnd;
    		var text = dom.value;
    		var begin = Math.max(text.lastIndexOf(' ', start - 1), text.lastIndexOf('\n', start - 1));
    		var end = Math.max(text.indexOf(' ', start-1), text.indexOf('\n', start-1));
    		var keyword = text.substring(begin!=-1?(begin+1):0, end!=-1?end:text.length);
			
    		console.log(keyword);
    		if($.inArray(keyword.toUpperCase(), keywords) !== -1){
    			var newText = '';
    			if(begin !== -1){
    				newText += text.substring(0, begin + 1);
    			}
    			newText += keyword.toUpperCase();
    			if(end !== -1){
    				newText += text.substring(end);
    			}
    			dom.value = newText;
    			dom.selectionStart = start;
    			dom.selectionEnd = stop;
    		}
    	}
    </script>
</head>
