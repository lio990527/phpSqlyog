	...
	foreach ($sqls as $sql){
		if(preg_match('/^(update|delete|insert)/i', $sql)) {
			...
			if(!empty($_REQUEST['order'])){
				$sql = stripos($sql, 'limit') ? str_ireplace('limit', "ORDER BY {$_REQUEST['order']} {$_REQUEST['sort']} LIMIT", $sql) : $sql." ORDER BY {$_REQUEST['order']} {$_REQUEST['sort']}";
				$result['order'] = $_REQUEST['order'];
				$result['sort']  = $_REQUEST['sort'];
			}
			if(stripos($sql, 'limit') === false && empty($_REQUEST['export'])){
